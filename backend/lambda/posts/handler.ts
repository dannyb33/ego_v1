...
import { BasePost, ImagePost, TextPost } from "./types";
...
type ImagePostResponse = Omit<ImagePost, 'PK' | 'SK' | 'entityType'> & {
    __typename: "ImagePost",
    uuid: String
}

type PostResponse = BasePostResponse | TextPostResponse | ImagePostResponse;
...
export const handler = async (event: any) => {
    ...
    switch (fieldName) {
        ...
        case 'createImagePost':
            return await createImagePost(sub, event.arguments.imageUrl, event.arguments.text);
        default:
...
const createImagePost = async (sub: any, imageUrl: string, text: string) => {
    try {
        ...
        throw new Error(`Failed to create image post: ${error}`);
    }
}
